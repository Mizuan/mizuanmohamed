...
return [
    ...
    |
    | We deliberately use the *root* domain (mizuan.dev), not the app URL host,
    | so a passkey stays valid across subdomains — auth lives on admin.mizuan.dev
    | but the passkey should remain usable if that ever moves.
    |
    */
    ...
    |--------------------------------------------------------------------------
    |
    | The origins permitted to complete WebAuthn ceremonies. Login happens on
    | the admin subdomain, so that origin must be listed alongside the main
    | site — otherwise the browser's origin check rejects the ceremony.
    |
    */
